...
gmMemChain::MemChunk* gmMemChain::NewChunk()
{
  ...
  else //No, allocate a new one
  {
    ...
    newChunk->m_lastAddress = (void*)((size_t)newChunk->m_minAddress + m_chunkSize);
    newChunk->m_curAddress = newChunk->m_minAddress;
...
void* gmMemChain::Alloc(unsigned int a_numElementsToAlloc)
{
  ...
  if(((size_t)m_currentChunk->m_curAddress + allocSize) > (size_t)m_currentChunk->m_lastAddress)
  {
    NewChunk();    
  }
  retPtr = m_currentChunk->m_curAddress;
  m_currentChunk->m_curAddress = (void*)((size_t)m_currentChunk->m_curAddress + allocSize);
