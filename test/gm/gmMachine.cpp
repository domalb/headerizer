...
gmType gmMachine::CreateUserType(const char * a_name, void* a_userData)
{
  ...
  //OMOD
  m_types[type].m_userData = a_userData;
...
//OMOD
void* gmMachine::GetTypeUserData(gmType a_type)
{
	return m_types[a_type].m_userData;
}
...
gmType gmMachine::GetTypeId(const char * a_typename) const
{
  ...
  {
    gmStringObject* name = m_types[id].m_name;
    if( strcmp(name->GetString(), a_typename) == 0 )
    {
      return id;
    }
  }
