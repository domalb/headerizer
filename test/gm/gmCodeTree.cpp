...
const char * gmGetOperatorTypeNameNice(gmCodeTreeNodeOperationType a_type)
{
	switch(a_type)
	{
	case CTNOT_INVALID : return "CTNOT_INVALID";
	case CTNOT_DOT : return ".";
	case CTNOT_UNARY_PLUS : return "++";
	case CTNOT_UNARY_MINUS : return "--";
	case CTNOT_UNARY_NOT : return "!";
	case CTNOT_UNARY_COMPLEMENT : return "^";
	case CTNOT_ARRAY_INDEX : return "[]";
	case CTNOT_TIMES : return "*";
	case CTNOT_DIVIDE : return "/";
	case CTNOT_REM : return "%";
	case CTNOT_ADD : return "+";
	case CTNOT_MINUS : return "-";
	case CTNOT_LT : return "<";
	case CTNOT_GT : return ">";
	case CTNOT_LTE : return "<=";
	case CTNOT_GTE : return ">=";
	case CTNOT_EQ : return "==";
	case CTNOT_NEQ : return "!=";
	case CTNOT_AND : return "&&";
	case CTNOT_OR : return "||";
	case CTNOT_BIT_OR : return "|";
	case CTNOT_BIT_XOR : return "^";
	case CTNOT_BIT_AND : return "&";
	case CTNOT_SHIFT_LEFT : return "<<";
	case CTNOT_SHIFT_RIGHT : return ">>";
	case CTNOT_ASSIGN : return "=";
	case CTNOT_ASSIGN_FIELD : return "=[]";
	default : break;
	}
	return "UNKNOWN OPERATOR TYPE";
};
...
static void PrintRecursiveNice(const gmCodeTreeNode * a_node, FILE * a_fp, bool a_firstCall, const char* a_crlf)
{
	if(a_node)
	{
		static int indent;
...
#define PRINTINDENT() {for(i = 0; i < indent; ++i) fprintf(a_fp, " ");}
#define PRINTCHILDREN() for(i = 0; i < GMCODETREE_NUMCHILDREN; ++i){if(a_node->m_children[i]){PrintRecursiveNice(a_node->m_children[i], a_fp, false,a_crlf);}}


			if (a_crlf) {PRINTINDENT();}

			bool bPrintChild = true;
			...
			if(a_node->m_type == CTNT_DECLARATION)
			{
				...
				switch(a_node->m_subType)
				{
				case CTNDT_PARAMETER :
					{
						bPrintChild = false;
						fprintf(a_fp, "param ");
						PrintRecursiveNice(a_node->m_children[0], a_fp, false, NULL);
						break;
					}
				case CTNDT_VARIABLE :
					{
						bPrintChild = false;
						fprintf(a_fp, "var(%d) ", a_node->m_subTypeType);
						PrintRecursiveNice(a_node->m_children[0], a_fp, false, NULL);
						break;
					}
				default : fprintf(a_fp, "UNKNOWN DECLARATION:"); break;
				}
			}
			else if(a_node->m_type == CTNT_STATEMENT)
			{
				...
				case CTNST_BREAK : fprintf(a_fp, "break"); break;
				case CTNST_CONTINUE : fprintf(a_fp, "continue" ); break;
				case CTNST_FOR :
					{
						bPrintChild = false;
						fprintf(a_fp, "for(" );
						PrintRecursiveNice(a_node->m_children[0], a_fp, false, NULL);
						fprintf(a_fp, "; " );
						PrintRecursiveNice(a_node->m_children[1], a_fp, false, NULL);
						fprintf(a_fp, "; " );
						PrintRecursiveNice(a_node->m_children[2], a_fp, false, NULL);
						fprintf(a_fp, ")" GM_NL );
						PrintRecursiveNice(a_node->m_children[3], a_fp, false, a_crlf);
					}
					break;
				case CTNST_FOREACH : fprintf(a_fp, "foreach(" ); break;
				case CTNST_WHILE : fprintf(a_fp, "while" ); break;
				case CTNST_DOWHILE : fprintf(a_fp, "dowhile" ); break;
				case CTNST_IF :
					{
						bPrintChild = false;
						fprintf(a_fp, "if (" );
						PrintRecursiveNice(a_node->m_children[0], a_fp, false, NULL);
						fprintf(a_fp, ")" GM_NL );
						PrintRecursiveNice(a_node->m_children[1], a_fp, false, a_crlf);
						if (a_node->m_children[2])
						{
							PRINTINDENT();fprintf(a_fp, "else" GM_NL );
							PrintRecursiveNice(a_node->m_children[2], a_fp, false, a_crlf);
						}
						break;
					}
				case CTNST_COMPOUND :
					{
						fprintf(a_fp, "{" GM_NL);
						bPrintChild = false;
						PrintRecursiveNice(a_node->m_children[0], a_fp, false, a_crlf);
						PRINTINDENT();fprintf(a_fp, "}");
						break;
					}
#if GM_USE_FORK
				case CTNST_FORK : fprintf(a_fp, "fork"); break;
#endif //GM_USE_FORK
				default : fprintf(a_fp, "UNKNOWN STATEMENT:"); break;
				}
			}
			else if(a_node->m_type == CTNT_EXPRESSION)
			{
				...
				case CTNET_OPERATION :
					{
						if(a_node->m_subTypeType < CTNOT_MAX)
						{
							gmCodeTreeNodeOperationType op = (gmCodeTreeNodeOperationType) a_node->m_subTypeType;
							bool bNoMath = (op == CTNOT_DOT || op == CTNOT_ARRAY_INDEX || op == CTNOT_ASSIGN_FIELD || op == CTNOT_ASSIGN);

							bPrintChild = false;
							if (!bNoMath) fprintf(a_fp, "(");
							PrintRecursiveNice(a_node->m_children[0], a_fp, false, NULL);
							if (!bNoMath) fprintf(a_fp, " ");
							fprintf(a_fp, "%s", gmGetOperatorTypeNameNice(op));
							if (a_node->m_children[1])
							{
								if (!bNoMath) fprintf(a_fp, " ");
								if (a_node->m_children[1]->m_type == CTNT_EXPRESSION && a_node->m_children[1]->m_subType == CTNET_FUNCTION)
								{
									fprintf(a_fp, a_crlf);
									PrintRecursiveNice(a_node->m_children[1], a_fp, false, a_crlf);
								}
						...
						{ 
							fprintf(a_fp, "UNKNOWN CTNET_OPERATION");
						}
						break;
					}
				...
				case CTNET_CONSTANT :
					{
						switch(a_node->m_subTypeType)
						{
						case CTNCT_INT : fprintf(a_fp, "%d", a_node->m_data.m_iValue); break;
						case CTNCT_FLOAT : fprintf(a_fp, "%f3.2f", a_node->m_data.m_fValue); break;
						case CTNCT_STRING : fprintf(a_fp, "`%s`", a_node->m_data.m_string); break;
						case CTNCT_NULL : fprintf(a_fp, "<null>"); break;
						default: fprintf(a_fp, "UNKNOWN CTNET_CONSTANT");
						}
						break;
					}

				//case CTNET_IDENTIFIER : fprintf(a_fp, "[%s]", a_node->m_data.m_string); break;
				case CTNET_IDENTIFIER : fprintf(a_fp, "%s", a_node->m_data.m_string); break;
				case CTNET_THIS : fprintf(a_fp, "this"); break;
				case CTNET_CALL :
					{
						bPrintChild = false;
						fprintf(a_fp, "call ");
				...
				case CTNET_TABLE : fprintf(a_fp, "table"); break;
				default : fprintf(a_fp, "UNKNOWN EXPRESSION:"); break;
				}
			}
			else
			{
				fprintf(a_fp, "UNKNOWN NODE TYPE");
			}
			if (a_crlf) fprintf(a_fp, a_crlf);
...
{
  if(a_fp != NULL && m_locked)
  {
    PrintRecursiveNice(g_codeTree, a_fp, true, GM_NL);
  }
}
